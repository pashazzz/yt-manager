...
│   ├── handlers/
│   │   ├── sections.go  # SectionHandler: GET/POST/DELETE /sections, GET /sections/:id/shows
│   │   ├── shows.go     # ShowHandler: POST/GET/DELETE /shows, GET /shows/:id, PATCH /shows/:id/section
...
| `DELETE` | `/sections/:id` | Удалить раздел (шоу мигрируют в Default) |
| `GET` | `/sections/:id/shows` | Шоу внутри раздела |
| `POST` | `/shows` | Добавить плейлист (с опциональным `sectionId`) |
| `GET` | `/shows/:id` | Шоу + его эпизоды |
| `DELETE` | `/shows/:id` | Удалить шоу и все эпизоды |
| `PATCH` | `/shows/:id/section` | Переместить шоу в другой раздел |
