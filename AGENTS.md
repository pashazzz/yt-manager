...
│   │   ├── section.go   # Section {ID, Name, OwnerID, IsDefault, UseThumb, CreatedAt}
│   │   ├── show.go      # Show {ID, Title, PlaylistURL, OwnerID, SectionID, OrderIndex, CreatedAt}
│   │   └── episode.go   # Episode {ID, ShowID, VideoID, Title, Duration, CurrentTime, IsWatched, OrderIndex}
│   ├── repository/
│   │   ├── profile_repo.go  # Upsert, FindByID
│   │   ├── section_repo.go  # Create, FindByOwner, FindByID, EnsureDefault, Delete
│   │   ├── show_repo.go     # Create, FindByOwner, FindBySection, FindByID, UpdateSection, Delete
...
│   │   ├── types/index.ts       # TypeScript-типы (Show, Episode, Section, ShowDetail, …)
│   │   ├── components/
│   │   │   ├── VideoPlayer.tsx  # YouTube IFrame + seekTo + heartbeat + мобильный overlay
│   │   │   ├── EpisodeList.tsx  # Сайдбар со списком эпизодов
│   │   │   ├── ShowCard.tsx     # Карточка шоу с меню перемещения
│   │   │   ├── SectionCard.tsx  # Карточка раздела
│   │   │   └── AddShowModal.tsx # Модалка добавления (c выбором раздела)
│   │   ├── pages/
│   │   │   ├── SectionsPage.tsx # Главная: разделы
│   │   │   ├── ShowsPage.tsx    # Список шоу выбранного раздела
│   │   │   └── ShowPage.tsx     # Страница просмотра: плеер + сайдбар
│   │   ├── App.tsx              # React Router: / (разделы), /sections/:id, /shows/:id
...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/sections` | Список разделов со статистикой (ShowCount, EpisodeCount, FirstVideoID) |
| `POST` | `/sections` | Создать раздел |
| `POST` | `/sections/:id/delete` | Удалить раздел (шоу мигрируют в Default) |
| `POST` | `/sections/:id/settings` | Обновить настройки раздела (например, режим фона) |
| `GET` | `/sections/:id/shows` | Шоу внутри раздела |
| `POST` | `/sections/reorder` | Изменить порядок сортировки разделов (drag-and-drop) |
...
| `POST` | `/shows/reorder` | Изменить порядок сортировки шоу внутри раздела (drag-and-drop) |
| `GET` | `/shows/:id` | Шоу + его эпизоды |
| `POST` | `/shows/:id/delete` | Удалить шоу и все эпизоды |
| `POST` | `/shows/:id/section` | Переместить шоу в другой раздел |
...
### Оформление разделов
- Разделы поддерживают два режима фона: градиент (дефолт) или обложка («thumbnail») первого видео в разделе.
- Приоритет выбора обложки: первое видео самого верхнего плейлиста → первое одиночное видео раздела → градиент (если пусто).
- Переключение режима сохраняется в поле `useThumb` модели `Section`.
