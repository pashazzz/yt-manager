...
│   │   ├── tag.go       # Tag {ID, Name, OwnerID, IsDefault, UseThumb, CreatedAt, OrderIndex}
│   │   ├── show.go      # Show {ID, Title, PlaylistURL, OwnerID, TagIDs []string, OrderIndex, CreatedAt}
│   │   └── episode.go   # Episode {ID, ShowID, VideoID, Title, Duration, CurrentTime, IsWatched, OrderIndex, TagIDs []string}
│   ├── repository/
│   │   ├── profile_repo.go  # Upsert, FindByID
│   │   ├── tag_repo.go      # Create, FindByOwner, FindByID, EnsureDefault, Delete
│   │   ├── show_repo.go     # Create, FindByOwner, FindByTag, FindByID, UpdateTags, Delete, UpdateOrder, EnsureSinglesShow
│   │   └── episode_repo.go  # BulkCreate, FindByShow, FindByID, UpdateProgress, UpdateTags, Delete, DeleteByShow, UpdateOrder, GetMaxOrderIndex, FindSinglesByTag, FindByOwnerID
│   ├── ytdlp/
│   │   └── ytdlp.go     # Client.FetchPlaylist() — вызывает yt-dlp binary
│   ├── handlers/
│   │   ├── tags.go      # TagHandler: GET/POST /tags, GET /tags/:id/shows, POST /tags/:id/delete, POST /tags/reorder, POST /tags/:id/episodes
│   │   ├── shows.go     # ShowHandler: GET/POST /shows, GET /shows/:id, POST /shows/:id/delete, POST /shows/:id/tags, POST /shows/:id/reverse, POST /shows/:id/episodes, POST /shows/:id/episodes/reorder
│   │   └── episodes.go  # EpisodeHandler: POST /episodes/:id/progress, POST /episodes/:id/tags, POST /episodes/:id/delete, GET /episodes
│   ├── middleware/
...
│   │   ├── types/index.ts       # TypeScript-типы (Show, Episode, Tag, TagItems, …)
│   │   ├── components/
│   │   │   ├── VideoPlayer.tsx  # YouTube IFrame + seekTo + heartbeat + мобильный overlay
│   │   │   ├── EpisodeList.tsx  # Сайдбар со списком эпизодов
│   │   │   ├── ShowCard.tsx     # Карточка шоу с меню управления тегами
│   │   │   ├── TagCard.tsx      # Карточка тега
│   │   │   └── AddShowModal.tsx # Модалка добавления (c выбором тега)
│   │   ├── pages/
│   │   │   ├── TagsPage.tsx     # Главная: теги
│   │   │   ├── ShowsPage.tsx    # Список шоу выбранного тега
│   │   │   └── ShowPage.tsx     # Страница просмотра: плеер + сайдбар
│   │   ├── App.tsx              # React Router: / (теги), /tags/:id, /shows/:id
...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/tags` | Список тегов со статистикой (ShowCount, EpisodeCount, FirstVideoID) |
| `POST` | `/tags` | Создать тег |
| `POST` | `/tags/:id/delete` | Удалить тег (элементы не удаляются, тег снимается) |
| `POST` | `/tags/:id/settings` | Обновить настройки тега (например, режим фона) |
| `GET` | `/tags/:id/items` | Шоу + одиночные видео внутри тега |
| `POST` | `/tags/reorder` | Изменить порядок сортировки тегов (drag-and-drop) |
| `POST` | `/tags/:id/episodes` | Добавить одиночное видео напрямую в тег (через неявное шоу) |
| `POST` | `/shows` | Добавить плейлист (с опциональным `tagIds`) или пустое кастомное шоу |
| `POST` | `/shows/reorder` | Изменить порядок сортировки шоу внутри тега (drag-and-drop) |
| `GET` | `/shows/:id` | Шоу + его эпизоды |
| `POST` | `/shows/:id/delete` | Удалить шоу и все эпизоды |
| `POST` | `/shows/:id/tags` | Обновить теги шоу |
| `POST` | `/shows/:id/reverse` | Изменить сортировку эпизодов (старые/новые) |
| `POST` | `/shows/:id/episodes` | Добавить одно видео вручную в кастомное шоу |
| `POST` | `/shows/:id/episodes/reorder` | Изменить порядок эпизодов кастомного шоу (drag-and-drop) |
| `POST` | `/episodes/:id/progress` | Установить прогресс или отметить как просмотренное/непросмотренное |
| `POST` | `/episodes/:id/tags` | Обновить теги одиночного эпизода |
| `POST` | `/episodes/:id/delete` | Удалить эпизод (видео) |
| `GET` | `/episodes` | Список всех эпизодов текущего профиля |
...
### Оформление тегов
- Теги поддерживают два режима фона: градиент (дефолт) или обложка («thumbnail») первого видео в теге.
- Приоритет выбора обложки: первое видео самого верхнего плейлиста → первое одиночное видео тега → градиент (если пусто).
- Переключение режима сохраняется в поле `useThumb` модели `Tag`.

### Мульти-теггинг
- Шоу и отдельные эпизоды поддерживают несколько тегов одновременно (`tagIds []string`).
- При добавлении нового контента через модалку можно выбрать несколько тегов.
- На карточках контента кнопка `#` позволяет добавлять/удалять теги из текущего списка.
- Если контент удаляется из последнего тега, он технически остаётся в БД, но может быть не виден в списках тегов (пока не будет назначен новый тег).
